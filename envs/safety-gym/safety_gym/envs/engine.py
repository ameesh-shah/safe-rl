...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    DEFAULT = {
        ...
        'placements_extents': [-2, -2, 2,
                               2],  # Placement limits (min X, min Y, max X, max Y)
        'placements_margin':
        0.0,  # Additional margin added to keepout when placing objects

        # Floor
        'floor_display_mode':
        False,  # In display mode, the visible part of the floor is cropped
        ...
        True,  # If true, end episode when resampling fails,
        ...
        'lidar_max_dist':
        None,  # Maximum distance for lidar sensitivity (if None, exponential distance)
        'lidar_exp_gain':
        1.0,  # Scaling factor for distance in exponential distance lidar
        ...
        'compass_shape':
        2,  # Set to 2 or 3 for XY or XYZ unit vector compass observation.
        ...
        'box_null_dist':
        2,  # Within box_null_dist * box_size radius of box, no box reward given
        ...
        'reward_distance':
        1.0,  # Dense reward multiplied by the distance moved to the goal
        ...
        'buttons_placements':
        None,  # Buttons placements list (defaults to full extents)
        ...
        'sensors_ball_joints':
        True,  # Observe named balljoint position / velocity sensors
        ...
        'constrain_indicator':
        True,  # If true, all costs are either 1 or 0 for a given step.

        # Hazardous areas
        'hazards_num': 0,  # Number of hazards in an environment
        'hazards_placements':
        None,  # Placements list for hazards (defaults to full extents)
        ...
        'vases_displace_cost':
        0.0,  # Cost (per step) per meter of displacement for a vase
        ...
        'pillars_placements':
        None,  # Pillars placements list (defaults to full extents)
        ...
        'gremlins_placements':
        None,  # Gremlins placements list (defaults to full extents)
    ...
    def build_observation_space(self):
        ...
        if self.observe_freejoint:
            obs_space_dict['freejoint'] = gym.spaces.Box(-np.inf,
                                                         np.inf, (7, ),
                                                         dtype=np.float64)
        if self.observe_com:
            obs_space_dict['com'] = gym.spaces.Box(-np.inf,
                                                   np.inf, (3, ),
                                                   dtype=np.float64)
        ...
        if self.task == 'push':
            if self.observe_box_comp:
                obs_space_dict['box_compass'] = gym.spaces.Box(-1.0,
                                                               1.0,
                                                               (self.compass_shape, ),
                                                               dtype=np.float64)
            if self.observe_box_lidar:
                obs_space_dict['box_lidar'] = gym.spaces.Box(0.0,
                                                             1.0,
                                                             (self.lidar_num_bins, ),
                                                             dtype=np.float64)
        if self.observe_goal_dist:
            obs_space_dict['goal_dist'] = gym.spaces.Box(0.0,
                                                         1.0, (1, ),
                                                         dtype=np.float64)
        if self.observe_goal_comp:
            obs_space_dict['goal_compass'] = gym.spaces.Box(-1.0,
                                                            1.0, (self.compass_shape, ),
                                                            dtype=np.float64)
        if self.observe_goal_lidar:
            obs_space_dict['goal_lidar'] = gym.spaces.Box(0.0,
                                                          1.0, (self.lidar_num_bins, ),
                                                          dtype=np.float64)
        if self.task == 'circle' and self.observe_circle:
            obs_space_dict['circle_lidar'] = gym.spaces.Box(0.0,
                                                            1.0,
                                                            (self.lidar_num_bins, ),
                                                            dtype=np.float64)
        if self.observe_remaining:
            obs_space_dict['remaining'] = gym.spaces.Box(0.0,
                                                         1.0, (1, ),
                                                         dtype=np.float64)
        if self.walls_num and self.observe_walls:
            obs_space_dict['walls_lidar'] = gym.spaces.Box(0.0,
                                                           1.0, (self.lidar_num_bins, ),
                                                           dtype=np.float64)
        if self.observe_hazards:
            obs_space_dict['hazards_lidar'] = gym.spaces.Box(0.0,
                                                             1.0,
                                                             (self.lidar_num_bins, ),
                                                             dtype=np.float64)
        if self.observe_vases:
            obs_space_dict['vases_lidar'] = gym.spaces.Box(0.0,
                                                           1.0, (self.lidar_num_bins, ),
                                                           dtype=np.float64)
        if self.gremlins_num and self.observe_gremlins:
            obs_space_dict['gremlins_lidar'] = gym.spaces.Box(0.0,
                                                              1.0,
                                                              (self.lidar_num_bins, ),
                                                              dtype=np.float64)
        if self.pillars_num and self.observe_pillars:
            obs_space_dict['pillars_lidar'] = gym.spaces.Box(0.0,
                                                             1.0,
                                                             (self.lidar_num_bins, ),
                                                             dtype=np.float64)
        if self.buttons_num and self.observe_buttons:
            obs_space_dict['buttons_lidar'] = gym.spaces.Box(0.0,
                                                             1.0,
                                                             (self.lidar_num_bins, ),
                                                             dtype=np.float64)
        if self.observe_qpos:
            obs_space_dict['qpos'] = gym.spaces.Box(-np.inf,
                                                    np.inf, (self.robot.nq, ),
                                                    dtype=np.float64)
        if self.observe_qvel:
            obs_space_dict['qvel'] = gym.spaces.Box(-np.inf,
                                                    np.inf, (self.robot.nv, ),
                                                    dtype=np.float64)
        if self.observe_ctrl:
            obs_space_dict['ctrl'] = gym.spaces.Box(-np.inf,
                                                    np.inf, (self.robot.nu, ),
                                                    dtype=np.float64)
        if self.observe_vision:
            width, height = self.vision_size
            rows, cols = height, width
            self.vision_size = (rows, cols)
            obs_space_dict['vision'] = gym.spaces.Box(0,
                                                      1.0,
                                                      self.vision_size + (3, ),
                                                      dtype=np.float64)
        ...
        if self.gremlins_num:
            ...
            for i in range(self.gremlins_num):
                ...
                object = {
                    ...
                    'pos': np.r_[self.layout[name.replace('obj', '')],
                                 self.gremlins_size],
        ...
        if self.hazards_num:
            for i in range(self.hazards_num):
                ...
                geom = {
                    ...
                    'pos': np.r_[self.layout[name],
                                 2e-2],  #self.hazards_size / 2 + 1e-2],
                    'rot': self.random_rot(),
                    'type': 'cylinder',
                    'contype': 0,
                    'conaffinity': 0,
        ...
        if self.gremlins_num:
            for i in range(self.gremlins_num):
                ...
                mocap = {
                    ...
                    'size': np.ones(3) * self.gremlins_size,
                    'type': 'box',
                    'pos': np.r_[self.layout[name.replace('mocap', '')],
                                 self.gremlins_size],
                    'rot': self._gremlins_rots[i],
                    'group': GROUP_GREMLIN,
    ...
    def obs_vision(self):
        ...
        return np.array(vision, dtype='float64') / 255
    ...
    #             for sensor in self.robot.hinge_pos_names:
    #                 theta = float(
    #                     self.world.get_sensor(sensor))  # Ensure not 1D, 1-element array
    #                 obs[sensor] = np.array([np.sin(theta), np.cos(theta)])
    #             for sensor in self.robot.ballquat_names:
    #                 quat = self.world.get_sensor(sensor)
    #                 obs[sensor] = quat2mat(quat)
    ...
    def cost(self):
        ...
        buttons_constraints_active = self.constrain_buttons and (self.buttons_timer
                                                                 == 0)
        ...
        if self.constrain_vases and self.vases_displace_cost:
            ...
            for i in range(self.vases_num):
                ...
                dist = np.sqrt(
                    np.sum(
                        np.square(
                            self.data.get_body_xpos(name)[:2] -
                            self.reset_layout[name])))
        ...
        if self.constrain_hazards:
            ...
            for h_pos in self.hazards_pos:
                ...
                if h_dist <= self.hazards_size:
                    cost['cost_hazards'] += self.hazards_cost * (self.hazards_size -
                                                                 h_dist)
    ...
    def step(self, action):
        ...
        exception = False
        for _ in range(self.rs.binomial(self.frameskip_binom_n,
                                        self.frameskip_binom_p)):
            try:
                self.set_mocaps()
    ...
    def reward(self):
        ...
        if self.task == 'push':
            ...
            gate_dist_box_reward = (self.last_dist_box >
                                    self.box_null_dist * self.box_size)
            reward += (self.last_dist_box -
                       dist_box) * self.reward_box_dist * gate_dist_box_reward
        ...
        if self.task == 'circle':
            ...
            reward += (((-u * y + v * x) / radius) /
                       (1 + np.abs(radius - self.circle_radius))) * self.reward_circle
    ...
    def render(self,
        ...
        if self.render_lidar_markers:
            ...
            if 'circle_lidar' in self.obs_space_dict:
                self.render_lidar([ORIGIN_COORDINATES], COLOR_CIRCLE, offset,
                                  GROUP_CIRCLE)
                offset += self.render_lidar_offset_delta
            ...
            if 'gremlins_lidar' in self.obs_space_dict:
                self.render_lidar(self.gremlins_obj_pos, COLOR_GREMLIN, offset,
                                  GROUP_GREMLIN)
                offset += self.render_lidar_offset_delta
        ...
        # Add goal marker
        if self.task == 'button':
            self.render_area(self.goal_pos,
                             self.buttons_size * 2,
                             COLOR_BUTTON,
                             'goal',
                             alpha=0.1)
